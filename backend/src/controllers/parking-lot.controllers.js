...
const createParkingLotController = asyncHandler(async (req, res) => {
  const {
    name,
    description,
    address,
...
const getAllParkingLotsController = asyncHandler(async (req, res) => {
  const parkingLots = await getAllParkingLots();

  return res
    .status(200)
    .json(
      new ApiResponse(200, parkingLots, "Parking lots fetched successfully"),
    );
});

const getParkingLotByIdController = asyncHandler(async (req, res) => {
  const { id } = req.params;
...
const updateParkingLotController = asyncHandler(async (req, res) => {
  const { id } = req.params;

  const updatedParkingLot = await updateParkingLot(id, req.body);
...
const deleteParkingLotController = asyncHandler(async (req, res) => {
  const { id } = req.params;

  const deletedParkingLot = await deleteParkingLot(id);
