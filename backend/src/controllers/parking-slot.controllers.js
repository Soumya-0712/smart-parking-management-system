...
import {
  ...
  deleteParkingSlot,
} from "../services/parking-slot.services.js";

import { SLOT_STATUS } from "../constants/parking.constants.js";

const createParkingSlotController = asyncHandler(async (req, res) => {
  const { lotId, floorNumber, slotNumber, slotType } = req.body;

  // Validation
  if (!lotId?.trim() || !slotNumber?.trim() || !slotType?.trim()) {
    throw new ApiError(400, "Lot ID, Slot Number and Slot Type are required");
  }
  ...
  return res
    .status(201)
    .json(
      new ApiResponse(201, parkingSlot, "Parking slot created successfully"),
    );
});

const getParkingSlotsController = asyncHandler(async (req, res) => {
  ...
  const parkingSlots = await getParkingSlots(filters);

  return res
    .status(200)
    .json(
      new ApiResponse(200, parkingSlots, "Parking slots fetched successfully"),
    );
});

const getParkingSlotByIdController = asyncHandler(async (req, res) => {
  const { id } = req.params;
...
const updateParkingSlotController = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  const { floorNumber, slotNumber, slotType } = req.body;

  if (floorNumber === undefined && !slotNumber && !slotType) {
    throw new ApiError(400, "At least one field must be provided for update");
  }

  const updatedParkingSlot = await updateParkingSlot(id, {
    floorNumber,
    slotNumber,
    slotType,
  });
...
const deleteParkingSlotByIdController = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, deletedSlot, "Parking Slot deleted succcessfully"),
    );
});

export {
  createParkingSlotController,
  getParkingSlotsController,
  getParkingSlotByIdController,
  updateParkingSlotController,
  updateParkingSlotStatusController,
  deleteParkingSlotByIdController,
};
