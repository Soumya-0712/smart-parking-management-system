...
const createParkingLot = async (parkingLotData) => {
  const existingParkingLot = await prisma.parkingLot.findFirst({
    where: {
  ...
  if (existingParkingLot) {
    throw new ApiError(409, "Parking lot already exists");
  }
...
const updateParkingLot = async (parkingLotId, updateData) => {
...
const deleteParkingLot = async (parkingLotId) => {
  const existingParkingLot = await prisma.parkingLot.findUnique({
    where: {
      id: parkingLotId,
    },
  ...
  const deletedParkingLot = await prisma.parkingLot.update({
    where: {
      id: parkingLotId,
    },
    data: {
